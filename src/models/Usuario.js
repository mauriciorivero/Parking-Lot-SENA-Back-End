...
class Usuario {
  ...
  // Helper method to encrypt password using MD5
  _encryptPasswordMD5(password) {
  ...
  async create() {
    try {
      // Encrypt password before storing if password is provided
      const encryptedPassword = this._clave ? this._encryptPasswordMD5(this._clave) : null;
      ...
      const params = [
        ...
        this._foto_perfil, this._estado, encryptedPassword, this._perfil_usuario_id
      ];
      
      const result = await this._db.executeQuery(sql, params);
      this._id_usuario = result.results.insertId;
      // Store the encrypted password in the object
      this._clave = encryptedPassword;
      return this;
    } catch (error) {
  ...
  // LOGIN VALIDATION - Validate user credentials for login
  static async validateLogin(direccion_correo, clave) {
    const db = new DatabaseConnection();
    try {
      // Find user by email
      const sql = 'SELECT * FROM USUARIO WHERE direccion_correo = ?';
      const result = await db.executeQuery(sql, [direccion_correo]);
      ...
      // Encrypt provided password using MD5
      const encryptedInputPassword = crypto.createHash('md5').update(clave).digest('hex');
      
      // Compare encrypted passwords
      if (encryptedInputPassword === row.clave) {
        // Create user object without password for security
        const user = new Usuario();
        user._mapRowToObject(row);
        const userResponse = user.toJSON();
        delete userResponse.clave; // Remove password from response
  ...
  async update() {
    try {
      // Encrypt password before updating if password is provided and not already encrypted
      let passwordToUpdate = this._clave;
      if (this._clave && this._clave.length !== 32) { // MD5 hash is always 32 characters
        passwordToUpdate = this._encryptPasswordMD5(this._clave);
      }
      ...
      const params = [
        this._tipo_documento, this._numero_documento, this._primer_nombre, this._segundo_nombre,
        this._primer_apellido, this._segundo_apellido, this._direccion_correo, this._numero_celular,
        this._foto_perfil, this._estado, passwordToUpdate, this._perfil_usuario_id, this._id_usuario
      ];
      
      await this._db.executeQuery(sql, params);
      // Update the object with the encrypted password
      this._clave = passwordToUpdate;
      return this;
    } catch (error) {
